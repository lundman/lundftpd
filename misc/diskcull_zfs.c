...
#ifdef LINUX
#define _LIBSPL_UNISTD_H 1
#define stat64 stat
#endif

// Linux build
// gcc -o diskcull diskcull.c -DLINUX -DZFS -I/usr/include -I/usr/include/x86_64-linux-gnu/ -I/usr/local/include/libzfs -I/usr/local/include/libspl/ -ldl
...
#if __STDC__
void logx(char *path, char const *fmt, ...)
#else
void logx(path, fmt, va_alist)
  char *path;
  char *fmt;
  va_dcl
#endif
...
#ifdef LINUX
  if ((hdl = dlopen("/usr/local/lib/libzfs.so", RTLD_LAZY)) != NULL) {
#else
  if ((hdl = dlopen("/usr/local/lib/libzfs.so", RTLD_LAZY)) != NULL) {
#endif
    ...
    if (_libzfs_init != NULL) {
      ...
      printf("Failed to dlopen libzfs.so: %s\n", dlerror());
  }
...
char *path2dataset(char *path)
{
    ...
    if (!fd) return NULL;
    while (getmntent_r(fd, &mp, buf, sizeof(buf))) {
        if (!strncmp(mp.mnt_dir, path, strlen(mp.mnt_dir))) {

            if (strlen(mp.mnt_dir) > longest) {
                longest = strlen(mp.mnt_dir);
                if (best) free(best);
                best = strdup(mp.mnt_fsname);
            }
        }
    }
...
static void
adjust_total_blocks(char *path, uint64_t *total,
		    uint64_t *avail,
		    uint64_t blocksize)
{
  char *dataset, *slash;
...
int enough_diskspace_full(struct delete_entry_struct *device)
{
...
#elif (defined (__sun__) || defined(LINUX)) && defined(ZFS)
  blksiz = sfsb.f_bsize;
  ...
  if (verbose)
#if defined ( IRIX ) || defined ( HPUX ) || defined (_SGI_SOURCE) || defined(__sun__) || defined (LINUX)
  ...
  */
  ...
  while(!enough_diskspace(device)) {
	...
	printf("Deleting '%s'\n", rmpath);
      ...
      if (strlen(rmpathl) >= 50) { /* truncate if it's too long */
	rmpathl[49] = '~';
	rmpathl[50] = 0;
      }
	...
	if (device->log_path) logx(device->log_path,
				  "%-40s : FAILED - %s\n",
				  rmpathl,
				  sys_errlist[errno]);
	...
	if (device->log_path) logx(device->log_path,
      ...
      if (verbose)
#if defined ( IRIX ) || ( HPUX ) || (_SGI_SOURCE) || defined(__sun__) || defined(LINUX)
